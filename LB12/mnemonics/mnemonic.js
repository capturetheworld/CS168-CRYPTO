...
class Mnemonic {
  ...
  calculateSequence(words) {
    ...
    for(let word in wordArray){
      if(wordArray[word]!==''){
        bits += this.constructor.translate11bit(this.wordlist.indexOf(wordArray[word]));
      }
    }
    ...
    for (var bit = 0; bit < bits.length; bit++) {
    	if(bytes[bit/8] !== undefined){
        bytes[bit/8] += bytes[bit];
    ...
    for(let byte in bytes){
    	this.seq.writeUInt8(this.constructor.convertBinStringToByte(bytes[byte]),parseInt(byte));
    }
  }
